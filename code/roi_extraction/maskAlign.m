...
function [ptr]=maskAlign(procSelector,setName,segmented)

preProcName = segmented.preProcName;
dbName = segmented.dbName;


ptr.preProcName = 'roi_extraction';
ptr.dbName = strcat('results/',dbName);

plotFlag = false; % set to true for vizualization (need to press button after each iteration)
saveResults = true; 
...
% preProcName = 'SETsegmentedWristImages'
% setName = {'SET5'};                    % image sets folder names

% set paths
pathImg = fullfile('../',dbName,preProcName,setName{1},'img')
pathBinMask = fullfile('../',dbName,preProcName,setName{1},'mask')

filesI = dir(pathImg); filesI = filesI(3:end);
filesM = dir(pathBinMask); filesM = filesM(3:end);
% mkdir(fullfile('../results',dbName,'roi_extraction',setName{1},'img'));
% mkdir(fullfile('../results',dbName,'roi_extraction',setName{1},'mask'));
% pathSave = fullfile('../results',dbName,'roi_extraction',setName{1},'img')
% pathSaveMask = fullfile('../results',dbName,'roi_extraction',setName{1},'mask')
if(procSelector == true)
    mkdir(fullfile('../results',dbName,'roi_extraction',strcat(setName{1},'p'),'img'));
mkdir(fullfile('../results',dbName,'roi_extraction',strcat(setName{1},'p'),'mask'));
pathSave = fullfile('../results',dbName,'roi_extraction',strcat(setName{1},'p'),'img')
pathSaveMask = fullfile('../results',dbName,'roi_extraction',strcat(setName{1},'p'),'mask')
else
    mkdir(fullfile('../results',dbName,'roi_extraction',setName{1},'img'));
mkdir(fullfile('../results',dbName,'roi_extraction',setName{1},'mask'));
pathSave = fullfile('../results',dbName,'roi_extraction',setName{1},'img')
...
% procSelector = true; % select true or false for either proc2 or proc2/3 for alignment
