% WRIST SKIN SEGMENTATION demo script
...
function [ptr]=maskDoSegmentation(select,img)

preProcName = img.preProcName;
dbName = img.dbName;

ptr.preProcName = 'segmentation';
ptr.dbName = strcat('results/',dbName);
% dbName = 'NTU-Wrist-Demo-Database' % database folder name
% preProcName = 'SEToriginalWristImages'
setName = {'SET1','SET2'};             % image sets folder names
addpath('../classifiersTrees');       % folder contsins ensemble of trees classifier
...
if(select == 1)
pathImg = fullfile('../',dbName,preProcName,setName{1})
mkdir(fullfile('../results',dbName,'segmentation',setName{1},'img'))
mkdir(fullfile('../results',dbName,'segmentation',setName{1},'mask'))
pathSave = fullfile('../results',dbName,'segmentation',setName{1},'img')
pathSave2 = fullfile('../results',dbName,'segmentation',setName{1},'mask')
tic
if(exist('Ensemble2_100_0110.mat')==0)
   ...
   fprintf('https://www.dropbox.com/s/zjkgms09zcf9eik/classifiersTrees.zip?dl=0'); 
end
...
if(select == 2)
pathImg = fullfile('../',dbName,preProcName,setName{2})
mkdir(fullfile('../results',dbName,'segmentation',setName{2},'img'))
mkdir(fullfile('../results',dbName,'segmentation',setName{2},'mask'))
pathSave = fullfile('../results',dbName,'segmentation',setName{2},'img') 
pathSave2 = fullfile('../results',dbName,'segmentation',setName{2},'mask')
tic
...
if(write2File == true)
        ...
        name = strcat(files(i).name(1:end-4),'.jpg');
